...
public class Computer extends Speler {
	 ...
	 */
	public Computer(int moeilijkheidsgraad, int aantalAambeelden,
			SpelBord bord, int xWaardeAambeeld) {
	...
	public void doeZetMoeilijk() {
		...
		if (hoogstePuntenMove.getMethode().equals("schijf")) {
			System.out.println("Computer koos kolom "
					+ hoogstePuntenMove.getKolom());
			this.bord.zetSchijf(hoogstePuntenMove.getKolom(), this.symbool);
		} else if (hoogstePuntenMove.getMethode().equals("aambeeld")) {
			System.out
					.println("Computer koos om een aambeeld te zetten op kolom "
							+ hoogstePuntenMove.getKolom());
			this.bord.zetAambeeld(hoogstePuntenMove.getKolom(),
					this.getxAambeeld(), this);
		} else {
			this.doeZetNormaal();
		}
	...
	public void doeZetMakkelijk() {
		...
		do {
			int randomInt = randomGenerator.nextInt(this.bord
					.getAantalKolommen()) + 1;
